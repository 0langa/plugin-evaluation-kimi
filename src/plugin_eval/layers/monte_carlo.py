...
SIMULATION_SCHEMA = {
    ...
    "required": ["activated", "quality_score", "response"],
...
async def run_simulation(
    ...
    """Run one explicit Codex skill-routing simulation."""
    try:
        response = await query_llm(
            f"""Decide whether Codex should activate the supplied skill for the user request.
If activated, provide a brief representative response that follows the skill and score its
expected correctness, completeness, and usefulness. If not activated, use quality_score 0.
...
{prompt}
            ...
            provider=provider,
            system="You are testing OpenAI Codex skill routing. Follow the JSON schema exactly.",
            schema=SIMULATION_SCHEMA,
            model=model,
        )
        result = response.data
        activated = bool(result["activated"])
        quality_score = float(result["quality_score"]) if activated else 0.0
...
class MonteCarloAnalyzer:
    ...
    async def _generate_prompts(self, name: str, description: str) -> list[str]:
        """Use Codex to generate 15 varied prompts. Falls back to basic variants."""
        try:
            system = (
                "You are generating routing tests for an OpenAI Codex skill. "
            ...
            prompt = (
                ...
                "Return 15 diverse positive prompts under the prompts key."
            )
            schema = {
                "type": "object",
                "properties": {
            ...
            if len(prompts) >= 5:
                return [str(item) for item in prompts[:15]]
        except Exception:
            pass
